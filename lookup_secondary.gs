function doGet(e) {
  ...
  function OnSearch(searchString) {
    var values = sheets.getSheetByName('Sheet2').getDataRange().getValues();
    for (var i = 1; i < values.length-1; i++) {
      ...
      for (var k = 0;k <=searcharr.length-1; k++){
        ...
        if (row[0].equalsIgnoreCase(word)) {
          var output = row[1];
          var params = JSON.stringify({'Category2': output, 'Matching word': word});
          return ContentService.createTextOutput(params).setMimeType(ContentService.MimeType.JSON);
        }
      }
    }
    return ContentService.createTextOutput(JSON.stringify({'Category2': 'null','Matching word': searchString})).setMimeType(ContentService.MimeType.JSON);
  }
